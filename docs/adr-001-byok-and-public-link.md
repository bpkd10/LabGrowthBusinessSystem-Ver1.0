...
ข้อ 10.1 (POST จริงพร้อม key ที่ใช้งานได้) **ปิดแล้ว 2026-08-09** เจ้าของระบบยิงด้วย key จริงบน `labgrowth.uncletungai.site` แล้วได้คำตอบวิเคราะห์กลับมาครบ เส้นทาง เบราว์เซอร์ → Worker → OpenAI → เบราว์เซอร์ ทำงานสมบูรณ์
...
## ภาคผนวก ง — โดเมนสาธารณะ (2026-08-09)

| โฮสต์ | ปลายทาง | สถานะ |
|---|---|---|
| `uncletungai.site` | Netlify | เว็บธุรกิจจริง **ห้ามแตะ** |
| `www.uncletungai.site` | Netlify (301 → apex) | เว็บธุรกิจจริง **ห้ามแตะ** |
| `labgrowth.uncletungai.site` | Worker | **ชื่อทางการเพียงชื่อเดียว** ของระบบนี้ |
| `crm.uncletungai.site` | — | ถอดออกแล้ว 2026-08-20 (ดูด้านล่าง) |

`custom_domain = true` สั่งให้ Wrangler **สร้าง** DNS record ใหม่ ไม่ใช่แก้ของเดิม record ของ apex และ www จึงไม่ถูกแตะ ตรวจก่อนผูกทุกครั้งว่าชื่อที่จะใช้ยังไม่มี record อยู่ (`dig +short <ชื่อ>.uncletungai.site`)

เลือกอยู่ใต้ `uncletungai.site` แทนการซื้อ apex ใหม่ (เคยพิจารณา `utai.site` ซึ่งตรวจแล้วว่ายังว่าง ไม่มีใครจด) เพราะระบบนี้ขอให้ผู้ใช้กรอกชื่อและเบอร์ลูกค้าจริง โดเมนจึงเป็นสัญญาณความน่าเชื่อถือด่านแรก การย้ายไป apex ที่ไม่มีใครรู้จักจะตัดความเชื่อมโยงกลับไปหาแบรนด์ที่ผู้เรียนไว้ใจอยู่แล้ว

`scripts/check-deploy-target.mjs` บังคับกฎนี้ พิสูจน์แล้วว่าหยุดได้จริงเมื่อมี route ครอบ apex หรือ www

### ง.1 ทำไมถอด `crm.uncletungai.site` ออก (2026-08-20)

เดิมผูก `crm` ไว้คู่กับ `labgrowth` โดยให้เหตุผลว่า "เก็บไว้ให้ลิงก์ที่ส่งออกไปแล้วไม่ตาย" ตรวจสอบภายหลังพบว่าเหตุผลนั้นตั้งอยู่บนสมมติฐานที่ไม่จริง — เจ้าของระบบยืนยันว่าไม่เคยส่งลิงก์ `crm` ออกไปเลย

และการเก็บไว้มีราคาที่มองไม่เห็น **สองชื่อโดเมนที่เสิร์ฟไฟล์ชุดเดียวกัน = คลังข้อมูลผู้ใช้สองใบที่มองไม่เห็นกัน** เพราะ `localStorage` ถูก browser ผูกไว้กับ origin ไม่ใช่กับเครื่อง วัดจากเครื่องเจ้าของระบบเองในวันที่ถอด:

| เปิดที่ | `business-growth-dashboard-demo` |
|---|---|
| `labgrowth.uncletungai.site` | มีข้อมูล 462 bytes |
| `crm.uncletungai.site` | ไม่มีเลย |

เบราว์เซอร์เดียวกัน คนเดียวกัน แต่เห็นคนละอย่าง ผู้ใช้ที่กรอกงานไว้ที่ชื่อหนึ่งแล้วเปิดอีกชื่อจะเจอแอปเปล่าและเข้าใจว่าข้อมูลหาย โดยไม่มีอะไรบนหน้าจออธิบายให้ — เป็นทางตันแบบเดียวกับที่ ADR นี้พยายามกำจัดมาตลอด

ตรวจก่อนถอดแล้วว่าไม่มีโค้ดหรือไฟล์ส่งออกใดอ้างถึงชื่อนี้ มีแต่ `wrangler.toml` และเอกสารฉบับนี้

**กฎที่ตามมา:** ระบบนี้มีชื่อทางการได้ชื่อเดียว ถ้าวันหนึ่งต้องย้ายชื่อ ให้ชื่อเก่าตอบ `301` ไปชื่อใหม่ อย่าเสิร์ฟไฟล์ซ้ำสองที่ `scripts/check-deploy-target.mjs` บังคับกฎนี้แล้ว และพิสูจน์แล้วว่าหยุดได้จริงเมื่อมี `pattern` ตัวที่สอง

**หมายเหตุการถอน:** การลบ route ออกจาก `wrangler.toml` แล้ว deploy จะหยุดผูก Worker กับชื่อนั้น แต่ Wrangler **ไม่ลบ Custom Domain กับ DNS record ที่สร้างค้างไว้ให้เอง** ต้องเข้า Cloudflare dashboard ลบด้วยมืออีกขั้น (Workers & Pages → Worker → Domains)

ทำจริงแล้วเมื่อ 2026-08-20 ตรวจยืนยันจากนอกระบบ: `crm.uncletungai.site` ตอบ NXDOMAIN, `labgrowth.uncletungai.site` ตอบ 200, `uncletungai.site` และ `www.uncletungai.site` ยังเป็น Netlify ตามเดิม (200 และ 301) การลบผ่าน dashboard ลบ DNS record ให้พร้อมกันเลย ไม่ต้องตามลบในหน้า DNS อีกขั้น

### ง.2 ปิด URL แถม `*.workers.dev` (2026-08-20)

ตอนถอด `crm` พบว่ายังมีประตูหลงเหลืออีกสองบานที่ไม่ได้ตั้งใจเปิด และไม่ได้อยู่ในรายการ `routes` เลย:

| ตัว | URL |
|---|---|
| Production | `utai-business-growth-system.<บัญชี>.workers.dev` |
| Preview | `*-utai-business-growth-system.<บัญชี>.workers.dev` |

ทั้งคู่ระบุไว้บน dashboard ว่า "Anyone with this URL can visit" และเสิร์ฟไฟล์ชุดเดียวกับโดเมนหลัก จึงเป็นคลังข้อมูลแยกใบด้วยเหตุผลเดียวกับ ง.1 ทุกประการ

`workers_dev = false` จึงถูกประกาศไว้ตรง ๆ ใน `wrangler.toml` **การลบบรรทัดนี้ทิ้งไม่เท่ากับการปิด** เพราะค่า default ของ Cloudflare คือเปิด — `scripts/check-deploy-target.mjs` จึงบังคับให้ต้องเขียน `false` ออกมาให้เห็น พิสูจน์แล้วว่าจับได้ทั้งกรณีตั้งเป็น `true` และกรณีลบบรรทัดทิ้ง

**ข้อแลกเปลี่ยนที่ยอมรับ:** ระบบไม่มีทางเข้าสำรองอีกต่อไป ถ้า `labgrowth.uncletungai.site` มีปัญหา DNS หรือใบรับรอง ทางแก้คือตั้ง `workers_dev = true` แล้ว deploy ใหม่ ซึ่งได้ URL สำรองคืนภายในไม่กี่นาที เลือกแบบนี้เพราะข้อมูลผู้ใช้หายเงียบ ๆ เป็นความเสียหายที่หนักกว่าและเกิดง่ายกว่าโดเมนหลักล่ม

---

## ภาคผนวก จ — วาง key แล้วใช้ได้ทันที (2026-08-09)

### จ.1 ปัญหาที่ผู้ใช้เจอจริง

วาง API key ในช่องแล้วถามคำถามทันที → ขึ้นว่า "ยังไม่ได้ตั้งค่า key" ทั้งที่ key อยู่ตรงหน้า

สาเหตุ: `readStoredKey()` อ่านจาก `sessionStorage`/`localStorage` เท่านั้น ค่าที่พิมพ์ในช่องถูกมองข้ามจนกว่าจะกด "บันทึกการตั้งค่า" และไม่มีอะไรบนหน้าจอบอกว่าต้องกดก่อน เป็นทางตันที่ผู้ใช้หาสาเหตุเองไม่ได้

ผลกระทบที่หนักกว่า: ผู้ใช้ที่**ไม่ต้องการให้ key ถูกเก็บที่ไหนเลย** ใช้ระบบไม่ได้เลย เพราะการใช้งานถูกผูกกับการบันทึก

### จ.2 สิ่งที่แก้

`activeApiKey()` = ค่าที่พิมพ์ไว้ **มาก่อน** ค่าที่บันทึกไว้ ผลคือ:

| การใช้งาน | key ถูกเก็บที่ไหน |
|---|---|
| วางแล้วถามเลย (ไม่กดอะไร) | **ไม่ถูกเก็บที่ไหนเลย** อยู่ในช่องกรอกของแท็บนั้น ปิดแท็บแล้วหาย |
| กดบันทึก ไม่ติ๊กจำ | `sessionStorage` หายเมื่อปิดแท็บ |
| กดบันทึก ติ๊กจำ | `localStorage` จนกว่าจะกดลบ |

ปุ่มบันทึกเปลี่ยนชื่อเป็น "บันทึกไว้ใช้ครั้งต่อไป" เพื่อสื่อว่าเป็นทางเลือก ไม่ใช่ขั้นตอนบังคับ

ยืนยันด้วยการทดสอบจริงในเบราว์เซอร์: วาง key → ปุ่มส่งเปิดใช้งานทันที → ส่งคำถามได้ → ได้ 401 จาก OpenAI จริง (พิสูจน์ว่า key ถูกส่งต่อครบเส้นทาง) → `sessionStorage` และ `localStorage` เป็น `null` ตลอดทั้งกระบวนการ

### จ.3 ความกังวลเรื่อง "เจ้าของระบบจะได้ key ไปไหม"

ตอบด้วยข้อเท็จจริงที่ผู้ใช้ตรวจสอบเองได้ ไม่ใช่คำรับรอง เพิ่มกล่อง "key ของคุณเดินทางไปไหนบ้าง" ในหน้า AI ที่ระบุ:

1. ไม่กดบันทึก = ไม่ถูกเขียนลงเครื่องเลย
2. ส่งผ่าน Worker ไป OpenAI เพราะ CORS ไม่ยอมให้เรียกตรง Worker ไม่เก็บ ไม่ log ไม่มี key ของตัวเอง
3. ไม่มีฐานข้อมูลฝั่งเซิร์ฟเวอร์เก็บอะไรทั้งสิ้น

**วิธีตรวจเอง:** F12 → Network → ส่งคำถาม เห็น `POST /api/ai-relay` รายการเดียว ยืนยันแล้วว่าตรงตามจริง

**ข้อจำกัดที่บอกตามตรงในหน้าจอ:** เมื่อ key วิ่งผ่านเซิร์ฟเวอร์ ผู้ใช้ต้องเชื่อใจว่าเจ้าของไม่ได้แอบบันทึก การอ้างว่า "ปลอดภัย 100%" จึงเป็นเท็จ สิ่งที่ทำได้คือบอกวิธีลดความเสี่ยงให้เหลือเท่าที่ควบคุมได้เอง: สร้าง Project API key แยก ตั้ง spend limit ต่ำ ๆ แล้วเพิกถอนหลังใช้ — ความเสี่ยงสูงสุดจึงเท่ากับวงเงินที่ตั้งไว้

### จ.4 บังคับด้วยชุดตรวจ

`scripts/check-ui-contract.mjs` ตรวจว่า `activeApiKey()` ให้ค่าที่พิมพ์มาก่อนเสมอ, การส่งคำถามไม่เรียก `readStoredKey()` ตรง ๆ, มีตัวดัก `input` เพื่ออัปเดตปุ่ม และเส้นทาง "วางแล้วใช้เลย" ไม่มี `setItem` พิสูจน์แล้วว่าด่านแดงจริงเมื่อย้อนโค้ดกลับ


---

## ภาคผนวก ฉ — ต้องบอกชื่อโมเดลที่ถูกเรียกใช้จริง (2026-08-09)

### ฉ.1 อาการ

กล่อง "โมเดลที่ใช้" แสดง `gpt-5.6-sol` แต่โมเดลที่ API เรียกใช้จริงไม่ใช่ตัวนั้น

### ฉ.2 สาเหตุ

`callProvider` คืน `data.model` ซึ่งเป็นชื่อโมเดลที่ผู้ให้บริการใช้จริงกลับมาอยู่แล้ว แต่โค้ดฝั่ง UI **ทิ้งค่านั้นทั้งหมด** หน้าจอจึงแสดงเฉพาะค่าที่ผู้ใช้พิมพ์ไว้เท่านั้น

OpenAI แปลงชื่อย่อเป็นรุ่นลงวันที่เสมอ (`gpt-5.6-sol` → `gpt-5.6-sol-2026-05-14`) ค่าทั้งสองจึงไม่มีทางตรงกันโดยธรรมชาติ

### ฉ.3 ทำไมถึงสำคัญกว่าที่เห็น

ระบบนี้ให้คำแนะนำทางธุรกิจที่เจ้าของเอาไปตัดสินใจจริง ถ้าไม่รู้ว่าคำตอบมาจากโมเดลรุ่นไหน จะเกิดสองปัญหา:

1. เปรียบเทียบคำตอบข้ามช่วงเวลาบนสมมติฐานที่ผิด — คำตอบที่ต่างกันอาจเป็นเพราะเปลี่ยนรุ่นโมเดล ไม่ใช่เพราะข้อมูลธุรกิจเปลี่ยน
2. ตรวจสอบย้อนหลังไม่ได้ว่าคำแนะนำที่ใช้ตัดสินใจไปแล้วมาจากอะไร

### ฉ.4 สิ่งที่แก้

แสดงชื่อจริงสองที่ และแสดงทั้งคู่เมื่อไม่ตรงกัน:

- ใต้ชื่อผู้ตอบในคำตอบ: `AI Business Analyst · ระบุ gpt-5.6-sol → ผู้ให้บริการใช้ gpt-5.6-sol-2026-05-14`
- แถบสถานะ key: `โมเดล gpt-5.6-sol · ใช้จริง gpt-5.6-sol-2026-05-14`

เมื่อผู้ใช้เปลี่ยนชื่อโมเดล ค่า "ใช้จริง" ของรอบก่อนถูกล้างทิ้ง ไม่ให้ค้างจนเข้าใจผิดว่าการเปลี่ยนไม่มีผล

### ฉ.5 คอมเมนต์เท็จที่แก้ไปพร้อมกัน

พบคอมเมนต์ใน `app/app.js` ที่ยังเขียนว่า *"ทั้งข้อมูลธุรกิจและ key ไม่วิ่งผ่าน server ของเจ้าของระบบอีกต่อไป"* ซึ่ง**เป็นเท็จตั้งแต่มีตัวส่งต่อ** (ภาคผนวก ก กลับข้อความนี้ไปแล้ว แต่คอมเมนต์ตกค้าง) คอมเมนต์ที่ขัดกับสถาปัตยกรรมจริงอันตรายกว่าไม่มีคอมเมนต์ เพราะคนอ่านจะเชื่อแล้วตัดสินใจผิด และมันยังขัดกับกล่อง "key ของคุณเดินทางไปไหนบ้าง" ที่บอกความจริงกับผู้ใช้ไว้แล้ว

`scripts/check-ui-contract.mjs` เพิ่มด่านห้ามข้อความนี้กลับมา และบังคับว่าต้องแสดงชื่อโมเดลที่ใช้จริง พิสูจน์แล้วว่าด่านแดงจริงทั้งสองกรณี
